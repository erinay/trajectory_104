...
Adegree = sum(A,2,'omitnan');     % create column vector of row-sums of adjacency matrix

DD = diag(Adegree);     % create diagonal degree matrix with row-sums of adjacency matrix

L = DD-A;               % create graph Laplacian

[eigvec, eigval] = eigs(L,DD);   % compute eigenvectors and eigenvalues of generalized eigenvalue problem. 
...
numeigvecs = 3;         % select number of eigenvectors to include in dendrogram analysis
